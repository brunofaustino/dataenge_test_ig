import os
import time
from pathlib import Path
from urllib.parse import urlparse
from src.data_processor import CommonCrawlProcessor
from src.website_categorizer import WebsiteCategorizer
from warcio.archiveiterator import ArchiveIterator
from bs4 import BeautifulSoup
import tldextract
import pandas as pd
import pyarrow as pa
import pyarrow.parquet as pq
import requests
import whois
import gzip
import io
import json
import logging
from typing import List, Dict, Any
from tqdm import tqdm
import zlib

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)
logger = logging.getLogger(__name__)

def get_latest_crawl() -> str:
    """
    Get the latest Common Crawl index.
    
    Returns:
        str: Latest crawl index
    """
    # Get the latest crawl index from the Common Crawl index
    response = requests.get("https://index.commoncrawl.org/collinfo.json")
    response.raise_for_status()
    crawls = response.json()
    latest_crawl = crawls[0]['id']
    return latest_crawl

def get_segment_urls(num_segments: int = 1) -> List[str]:
    """
    Get URLs for Common Crawl segments.
    
    Args:
        num_segments: Number of segments to get
        
    Returns:
        List of segment URLs
    """
    latest_crawl = get_latest_crawl()
    wat_paths_url = f"https://data.commoncrawl.org/crawl-data/{latest_crawl}/wat.paths.gz"
    
    # Download and extract the WAT paths
    response = requests.get(wat_paths_url)
    response.raise_for_status()
    
    # Decompress gzipped content
    with gzip.GzipFile(fileobj=io.BytesIO(response.content)) as gz:
        wat_paths = gz.read().decode('utf-8').strip().split('\n')
    
    # Select first N segments
    selected_paths = wat_paths[:num_segments]
    
    # Convert to full URLs
    base_url = "https://data.commoncrawl.org"
    return [f"{base_url}/{path}" for path in selected_paths]

def download_wat_file(url: str, output_path: Path) -> Path:
    """
    Download a WAT file with progress tracking.
    
    Args:
        url: URL of the WAT file
        output_path: Path to save the file
        
    Returns:
        Path to the downloaded file
    """
    logger.info(f"Starting download of WAT file from {url}")
    response = requests.get(url, stream=True)
    response.raise_for_status()
    
    # Get file size for progress bar
    file_size = int(response.headers.get('content-length', 0))
    
    progress_bar = tqdm(
        total=file_size,
        unit='iB',
        unit_scale=True,
        desc=f"Downloading {output_path.name}"
    )
    
    with open(output_path, 'wb') as f:
        for chunk in response.iter_content(chunk_size=8192):
            if not chunk:
                break
            f.write(chunk)
            progress_bar.update(len(chunk))
    
    progress_bar.close()
    logger.info(f"Successfully downloaded {file_size/1024/1024:.2f}MB to {output_path}")
    return output_path

def extract_links_from_wat(wat_path: Path) -> List[Dict[str, Any]]:
    """
    Extract external links from a WAT file with detailed progress tracking.
    
    Args:
        wat_path: Path to the WAT file
        
    Returns:
        List of dictionaries containing link information
    """
    links = []
    records_processed = 0
    records_with_links = 0
    total_links = 0
    errors = {'json': 0, 'domain': 0, 'link': 0, 'other': 0}
    
    logger.info(f"Starting to process WAT file: {wat_path}")
    
    try:
        # First pass to count lines for progress bar
        total_lines = 0
        with gzip.open(wat_path, 'rt', encoding='utf-8', errors='ignore') as f:
            for _ in f:
                total_lines += 1
        
        with gzip.open(wat_path, 'rt', encoding='utf-8', errors='ignore') as f:
            progress_bar = tqdm(total=total_lines, desc="Processing WAT records")
            
            for line in f:
                try:
                    records_processed += 1
                    record = json.loads(line)
                    envelope = record.get('Envelope', {})
                    payload = envelope.get('Payload-Metadata', {}).get('HTTP-Response-Metadata', {})
                    html_metadata = payload.get('HTML-Metadata', {})
                    
                    # Get source URL and domain
                    source_url = envelope.get('WARC-Header-Metadata', {}).get('WARC-Target-URI')
                    if not source_url:
                        continue
                        
                    try:
                        source_domain = tldextract.extract(source_url).domain
                    except Exception as e:
                        logger.debug(f"Error extracting source domain from {source_url}: {e}")
                        errors['domain'] += 1
                        continue
                    
                    # Extract links from HTML metadata
                    record_links = 0
                    for link in html_metadata.get('Links', []):
                        try:
                            href = link.get('href')
                            if not href or not href.startswith(('http://', 'https://')):
                                continue
                                
                            target_domain = tldextract.extract(href).domain
                            if not target_domain or target_domain == source_domain:
                                continue
                                
                            # Add homepage and subsection information
                            is_homepage_val = is_homepage(href)
                            subsection_val = get_subsection(href)
                            
                            links.append({
                                'source_url': source_url,
                                'target_url': href,
                                'source_domain': source_domain,
                                'target_domain': target_domain,
                                'is_homepage': is_homepage_val,
                                'subsection': subsection_val
                            })
                            record_links += 1
                            total_links += 1
                            
                        except Exception as e:
                            logger.debug(f"Error processing link {href}: {e}")
                            errors['link'] += 1
                            continue
                    
                    if record_links > 0:
                        records_with_links += 1
                            
                except json.JSONDecodeError as e:
                    logger.debug(f"Error decoding JSON at line {records_processed}: {e}")
                    errors['json'] += 1
                    continue
                except Exception as e:
                    logger.debug(f"Unexpected error processing record at line {records_processed}: {e}")
                    errors['other'] += 1
                    continue
                finally:
                    progress_bar.update(1)
                    
                # Print progress every 1000 records
                if records_processed % 1000 == 0:
                    logger.info(
                        f"Progress: {records_processed}/{total_lines} records processed, "
                        f"Found {total_links} links in {records_with_links} records"
                    )
            
            progress_bar.close()
            
    except Exception as e:
        logger.error(f"Fatal error processing WAT file: {e}")
        raise
    
    logger.info(f"WAT Processing Summary:")
    logger.info(f"- Total records processed: {records_processed}")
    logger.info(f"- Records with valid links: {records_with_links}")
    logger.info(f"- Total links extracted: {total_links}")
    logger.info(f"- Errors encountered:")
    logger.info(f"  - JSON decode errors: {errors['json']}")
    logger.info(f"  - Domain extraction errors: {errors['domain']}")
    logger.info(f"  - Link processing errors: {errors['link']}")
    logger.info(f"  - Other errors: {errors['other']}")
    
    return links

def is_homepage(url):
    """Check if a URL is a homepage."""
    path = urlparse(url).path
    return path in ("", "/", "/index.html", "/index.htm")

def get_subsection(url):
    """Extract subsection from URL."""
    path = urlparse(url).path
    parts = [p for p in path.split("/") if p]
    return parts[0] if parts else None

def main():
    """Main function to test the processor."""
    # Create data directory
    data_dir = Path("data")
    data_dir.mkdir(exist_ok=True)
    (data_dir / "raw").mkdir(exist_ok=True)
    
    # Get segment URLs
    segment_urls = get_segment_urls(num_segments=2)
    
    all_links = []
    # Process each segment
    for segment_url in segment_urls:
        # Download WAT file
        wat_file = data_dir / "raw" / Path(segment_url).name
        download_wat_file(segment_url, wat_file)
        
        # Extract links
        links = extract_links_from_wat(wat_file)
        all_links.extend(links)
        logger.info(f"Found {len(links)} links in {segment_url}")

    # Create data directory if it doesn't exist
    data_dir = Path("data")
    data_dir.mkdir(exist_ok=True)
    (data_dir / "processed").mkdir(exist_ok=True)
    (data_dir / "final").mkdir(exist_ok=True)
    
    # Initialize processor with database parameters
    db_params = {
        'dbname': 'crawldata',
        'user': 'dataengineer',
        'password': 'dataengineer',
        'host': 'localhost',
        'port': '5432'
    }
    
    processor = CommonCrawlProcessor(
        db_params=db_params,
        data_dir=data_dir
    )
    
    # Create DataFrame with all links
    links_df = pd.DataFrame(all_links)
    print(f"\nFound {len(links_df)} links in total")
    
    # Categorize websites
    print("\nCategorizing websites...")
    categorizer = WebsiteCategorizer(api_key=None)  # No API key for testing
    
    # Get unique domains for categorization
    unique_domains = links_df["target_domain"].unique()
    print(f"Found {len(unique_domains)} unique domains to categorize")
    
    # Process a sample of domains for testing
    sample_domains = unique_domains[:10]
    categories = []
    
    for domain in sample_domains:
        # Construct a URL for the domain
        url = f"https://{domain}"
        print(f"Processing {url}")
        
        try:
            # Get WHOIS data for country
            domain_info = whois.whois(domain)
            country = domain_info.country if isinstance(domain_info.country, str) else None
            
            # Get content category (placeholder for API call)
            category = None
            
            # Check if domain is ad-based
            is_ad_based = domain in categorizer._load_ad_domains()
            
            categories.append({
                "domain": domain,
                "country": country,
                "category": category,
                "is_ad_based": is_ad_based
            })
            
            # Sleep to avoid rate limiting
            time.sleep(1)
        except Exception as e:
            print(f"Error categorizing {domain}: {e}")
            categories.append({
                "domain": domain,
                "country": None,
                "category": None,
                "is_ad_based": False
            })
    
    # Create DataFrame with categorization results
    categories_df = pd.DataFrame(categories)
    
    # Merge link and category information
    result_df = links_df.merge(
        categories_df,
        left_on="target_domain",
        right_on="domain",
        how="left"
    )
    
    # Compute metrics
    print("\nComputing metrics...")
    metrics = {
        "total_unique_domains": result_df["target_domain"].nunique(),
        "homepage_ratio": (result_df["is_homepage"].sum() / len(result_df)) * 100,
        "avg_subsections_per_domain": result_df.groupby("target_domain")["subsection"].nunique().mean(),
        "top_categories": result_df["category"].value_counts().head(10).to_dict(),
        "geographic_distribution": result_df["country"].value_counts().head(10).to_dict()
    }
    
    # Print metrics
    print("\nMetrics:")
    for metric, value in metrics.items():
        print(f"{metric}: {value}")
    
    print("\nDataFrame Info:")
    print(result_df.info())
    print("\nDataFrame Sample:")
    print(result_df.head())

    # Ensure correct data types
    result_df['is_homepage'] = result_df['is_homepage'].fillna(False)
    result_df['is_ad_based'] = result_df['is_ad_based'].fillna(False)
    result_df['subsection'] = result_df['subsection'].fillna('')
    result_df['country'] = result_df['country'].fillna('')
    result_df['category'] = result_df['category'].fillna('')

    # Save to PostgreSQL
    print("\nSaving to PostgreSQL...")
    processor.load_to_postgres(result_df, "external_links")
    
    # Save metrics to PostgreSQL
    metrics_df = pd.DataFrame([metrics])
    processor.load_to_postgres(metrics_df, "website_metrics")
    
    # Save to Arrow/Parquet
    print("\nSaving to Arrow/Parquet...")
    table = pa.Table.from_pandas(result_df)
    pq.write_to_dataset(
        table,
        root_path=str(data_dir / "final"),
        partition_cols=["country", "category"]
    )
    
    print("\nProcessing complete!")

if __name__ == "__main__":
    main() 